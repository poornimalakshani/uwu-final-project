...
<div class="row">
	<div class="col-md-12">
	<h4>
		Income <?=(!empty($peopleId)) ? 'of '.$people->fullName : '' ?>
		<?php if(!empty($peopleId)) { ?>
		<span class="pull-right"><small><a href="/grama_niladhari/income/add_edit/<?=$homeId?>/<?=$peopleId?>" data-type="add" class="btn btn-success add-edit">Add New</a></small></span>
		<?php } ?>
	</h4>
	</div>
	<div class="col-md-12">
		...
		<?php if(empty($income)) { ?>
			<p class="text-warning"><?=(empty($peopleId) ? 'Please Select A People!' : 'Sorry, No Income Found!')?></p>
		<?php } else { ?>
		<table class="table table-striped">
		    ...
		    <?php foreach($income as $x) { ?>
				<tr>
					<td><?=$x->income?></td>
					<td align="right">
						<a class="action add-edit" data-type="edit" href="/grama_niladhari/income/add_edit/<?=$homeId?>/<?=$peopleId?>/<?=$x->id?>"><i class="fa fa-pencil fa-lg"></i></a>
						<a class="action delete" href="/grama_niladhari/income/delete/<?=$x->id?>"><i class="fa fa-trash fa-lg"></i></a>
					</td>
				</tr>
			<?php } ?>
			</tbody>
...
$(document).ready(function() {
	$('#home-select').on('change', function(e) {
		...
		window.location.href = "/grama_niladhari/income/"+urlPart;
	});
